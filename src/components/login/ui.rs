...
pub fn ui(frame: &mut Frame, _app: &mut App, connection_form: &mut ConnectionForm) {
    ...
    let current_editing_mode = match connection_form.mode {
        EditMode::Normal => {
            Span::styled("NORMAL", Style::default().fg(ratatui::style::Color::Green))
        }
        EditMode::Insert => Span::styled("INSERT", Style::default().fg(ratatui::style::Color::Red)),
    };

    frame.render_widget(connection_form, main_layout[1]);

    frame.render_widget(
        Paragraph::new(current_editing_mode).block(Block::new().borders(Borders::all())),
        outer_layout[1],
    );
}
